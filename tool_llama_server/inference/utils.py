...
def react_parser_parallel(string):
    if "Thought:" in string:
        thought = string[string.find("Thought:") + len("Thought:") : string.find("Function Call:")]
    else:
        thought = string[: string.find("Function Call:")]

    function = string[string.find("Function Call:") + len("Function Call:") :]
    return thought.strip(), parse_tool_call(function.strip())
...
def fix_json_string(json_str):
    ...
    left_quotes = json_str.count('"')
    right_quotes = left_quotes  # 双引号数量应该成对，所以右边补齐

    # 补齐右边的符号
    if left_brackets > right_brackets:
        json_str += "]" * (left_brackets - right_brackets)
...
def extract_name_arguments(json_str):
    tag_name = '"name":'
    tag_arguments = '"arguments":'
    ind = json_str.find(tag_arguments)
    name_matches = json_str[json_str.find(tag_name) + len(tag_name) : ind].strip()
    arguments_matches = json_str[ind + len(tag_arguments) :].strip()
    return name_matches, arguments_matches
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # 提取 "name" 和 "arguments" 后面的内容
                name, arguments = extract_name_arguments(json_str)
...
def generate_stream(model, tokenizer, params, device, context_len=8192, stream_interval=2, force_generate=False):

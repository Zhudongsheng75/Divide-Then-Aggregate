        ...
        if self.sep_style == SeparatorStyle.ONLY_LAST_ASSISTANT:
            ...
            for role, message in self.messages:
                if message:
                    ...
                    ret += role + ": " + "<s>"
            return ret
        elif self.sep_style == SeparatorStyle.ASSISTANT_PARALLEL:
            seps = [self.sep, self.sep2]
            ret = ""
            for i, (role, message) in enumerate(self.messages):
                if not message:
...
register_conv_template(
    Conversation(
        name="tool-llama-parallel",
        system="",
        roles=("System", "User", "Function", "Assistant"),
        messages=(),
        offset=0,
        sep_style=SeparatorStyle.ASSISTANT_PARALLEL,
        sep="\n",
        sep2="</s>",
    )
)
